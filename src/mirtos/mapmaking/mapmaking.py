...
def bin_map(frame, lat, lon, tods, center_ra, center_dec, npix_x=128, npix_y=128, pixel_size_deg=4 / 3600,
            projection="SIN"):
    if frame == 'RADEC':
        ...
        y_bins = np.linspace(center_dec_deg - npix_x // 2 * pixel_size, center_dec_deg + npix_x // 2 * pixel_size,
                             npix_y)
        ...
        data_map, x_edge, y_edge, binnumber = binned_statistic_2d(np.rad2deg(lat),
                                                                  ...
                                                                  statistic="mean",
                                                                  bins=[npix_x, npix_y],
                                                                  range=((x_bins[0] - center_ra_deg,
                                                                          x_bins[-1] - center_ra_deg),
                                                                         (y_bins[0] - center_dec_deg,
                                                                          y_bins[-1] - center_dec_deg)))
