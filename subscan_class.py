...
class Subscan():
    ...
    def extract_data(self, filename, config):
        '''
        This function extract the data from the fits file, normalize all the timestream for the optical responsivity
        ...
        if self.mode == 'SRT':
            ...
            #self.srp_tz = np.mean(hdul['SERVO TABLE'].data['SRP_TZ']) #mm
            #self.srp_ty = np.mean(hdul['SERVO TABLE'].data['SRP_TY']) #mm
            #self.srp_tx = np.mean(hdul['SERVO TABLE'].data['SRP_TX']) #mm
            ...
            try: 
                ...
                '''
                angle_offset = 0 #np.pi/2
                self.par_angle = hdul['DATA TABLE'].data['par_angle'][flag_track] + angle_offset
                self.ra_scan = hdul['DATA TABLE'].data['raj2000'][flag_track] 
                self.dec_scan = hdul['DATA TABLE'].data['decj2000'][flag_track]
                self.time = hdul["DATA TABLE"].data["time"][flag_track]
                self.ra_center = hdul['PRIMARY'].header['HIERARCH RightAscension']
                self.dec_center = hdul['PRIMARY'].header['HIERARCH Declination']
                '''
            except:
            ...
            self.timestream_raw = []
            #self.timestream_raw = hdul['PH TABLE'].data[hdr[0]][flag_track]
            for i in range(0, self.num_feed):
                #try:
                #self.timestream_raw.append(hdul['PH TABLE'].data[hdr[i]][flag_track])
