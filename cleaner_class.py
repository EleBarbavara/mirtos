...
def compute_common_mode(ts, pixel_mask, plot=False):
    #create the clipped timestreams (nan at each clipped element in the timestream)
        ...
        #print('1/3 : Compunting the timestream on which compute the common mode.....')
        for tod, mask in zip(noise_ts, pixel_mask):
            tod[np.logical_not(mask)]=np.nan
        
        #fcompute the average -> common mode
        #print('2/3 : Compunting the common mode.....')
...
class Cleaner():
    '''
        This class refers to the data filteirng. Its methods compute the general and customized common mode for each feed in order to minimize the noise 
        (removing the pulse tube noise). The correlation matrixes are shown in order to see the progress done during this analysis.
        Then, a map of the filtered data is shown + a map in the region of high coverage of the scan.
        '''
    def __init__(self, cfg):
        ...
        if all(mode == True for mode in [self.cfg['filtering']['gen_cm'], self.cfg['filtering']['cust_cm']]):
            raise ValueError('Choose one method to filter the data: either with the general common mode or the custom common mode.')
    ...
    def init_cleaner(self):    
        if all(mode == True for mode in [self.cfg['filtering']['gen_cm'], self.cfg['filtering']['cust_cm']]):
            ...
            print('-----> You choose to not filter the data, are you sure? :)')
            self.cm_type = 'not_filtered'
            
        elif self.cfg['filtering']['gen_cm'] == True:
            print('Filtering the data with the general common mode.')
            self.cm_type =  'gen_cm'
                
        elif self.cfg['filtering']['cust_cm'] == True:
            print('Filtering the data with the customized common mode.')
            self.cm_type = 'cust_cm'
    ...
    def filter(self, subscan, tsdt, pixel_mask):
        ...
        if all(mode == True for mode in [self.cfg['filtering']['gen_cm'], self.cfg['filtering']['cust_cm']]):
            ...
            raise ValueError('What are you doing with the config file? Check the filtering section.')
